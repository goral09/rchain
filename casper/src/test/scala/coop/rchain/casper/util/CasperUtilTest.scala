...
class CasperUtilTest extends FlatSpec with Matchers with BlockGenerator with BlockStoreFixture {
  ...
  "isInMainChain" should "classify appropriately" in withStore[Id, Assertion] {
    implicit blockStore =>
      ...
      val chain = createChain[StateWithChain].runS(initState)(syncId)
  ...
  "isInMainChain" should "classify diamond DAGs appropriately" in withStore[Id, Assertion] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      def createChain[F[_]: Monad: BlockDagState: Time: BlockStore]: F[BlockMessage] =
        for {
  ...
  "isInMainChain" should "classify complicated chains appropriately" in withStore[Id, Assertion] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
