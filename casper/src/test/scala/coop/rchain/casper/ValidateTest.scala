...
class ValidateTest
  ...
  "Block signature validation" should "return false on unknown algorithms" in withStore[Id, Unit] {
    implicit blockStore =>
  ...
  it should "return false on invalid ed25519 signatures" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
  ...
  it should "return true on valid ed25519 signatures" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val n                        = 6
  ...
  "Timestamp validation" should "not accept blocks with future time" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
  ...
  it should "not accept blocks that were published before parent time" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState)
  ...
  it should "return false for non-sequential numbering" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState)
      val block                    = chain.idToBlocks(1)
  ...
  it should "return true for sequential numbering" in withStore[Id, Unit] { implicit blockStore =>
    implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
    val n                        = 6
  ...
  "Block summary validation" should "short circuit after first invalidity" in withStore[Id, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState)
      val block                    = chain.idToBlocks(1)
